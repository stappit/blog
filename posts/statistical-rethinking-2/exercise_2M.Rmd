...
Here's my solutions to the medium exercises in chapter 2 of McElreath's Statistical Rethinking, 1st edition. My intention is to move over to the 2nd edition when it comes out next month.
...
```{r, echo=F, fig.cap="Solution 2M1"}
m1 %>% 
  ggplot() + 
  aes(p, posterior) + 
  geom_area(fill = 'skyblue', colour = 'white') +
  scale_x_continuous(labels = percent_format()) +
  geom_vline(xintercept = p_true, linetype = 'dashed', colour = 'chocolate') +
  facet_wrap(~dataset) +
  labs(
    x = 'Fraction of water, p',
    y = 'Posterior',
    title = "2M1: Posteriors for different datasets",
    subtitle = str_glue("Dashed line indicates ground truth p = {p_true}")
  )
```
...
m2 %>% 
  ...
  labs(
    x = 'Fraction of water, p',
    y = 'Posterior',
    title = "2M2: Effect of zero prior mass below 0.5",
    subtitle = str_glue("Dashed line indicates ground truth p = {p_true}")
  )
```
