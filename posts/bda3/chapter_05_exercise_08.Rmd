...
```{r prior_plot, echo = FALSE, fig.cap="conjugate mixture prior"}
prior %>% 
  ggplot() +
  aes(theta, density) +
  ...
  labs(
    title = 'Conjugate mixture prior for θ',
    x = 'θ',
    y = 'Density'
  )
```
...
This gives us positive and negative means of `r signif(c(mu_10_pos, mu_10_neg), digits = 3)`, respectively, and stanard deviations both equal to `r signif(sd_10_pos, digits = 3)`.
...
This gives positive and negative weights of `r percent(c(lambda_prime_pos, lambda_prime_neg))`, respectively.
...
posterior %>% 
  ...
  labs(
    x = 'θ',
    y = 'Density',
    title = 'Full posterior density of θ'
  )
```
