...
```{r, include = FALSE}
ps <- pbeta(c(0.005, 0.008), medium$alpha, medium$beta)
extremes <- c(ps[1], 1 - ps[2]) %>% percent()
```

The medium prior has probability mass `r extremes` below and above 0.5%, 0.8%, respecitvely, with a mean value of 0.65%. The low and high priors have a lower precision but with means that are lower and higher than 0.65%, respectively. They also overlap somewhat with the medium prior, but with lower density. This overlapping could allow the possibility that Nevada/Utah don't have such extreme values in the year of observation.

```{r priors_plot, echo = FALSE}
tibble(p = seq(0, 0.02, 0.0001)) %>% 
  mutate(
    low = dbeta(p, low$alpha, low$beta),
    medium = dbeta(p, medium$alpha, medium$beta),
    high = dbeta(p, high$alpha, high$beta)
  ) %>% 
  gather(type, density, -p) %>% 
  ggplot() +
  aes(p, density, colour = type) +
  geom_line() +
  scale_x_continuous(labels = percent) +
  labs(
    x = 'Divorce rate',
    y = 'Density',
    title = 'Priors for low, medium, and high divorce rates',
    subtitle = str_glue(paste(
      'Low: Beta({low$alpha}, {low$beta})',
      'Medium: Beta({medium$alpha}, {medium$beta})',
      'High: Beta({high$alpha}, {high$beta})',
      sep = '\n'
    )),
    colour = 'Type'
  )
...
posterior %>% 
  filter(p < 0.02) %>% 
  ggplot() +
  aes(p, density, fill = p < 0.0139) +
  geom_area(colour = 'white') +
  scale_x_continuous(labels = percent) +
  labs(
    title = 'Posterior for eighth state given the first seven',
    subtitle = str_glue('P(rate > 1.39%) = {extreme}'),
    x = 'Divorce rate',
    y = 'Density',
    fill = '< 1.39%'
  )
