...
```{r joint_plot, echo = FALSE}
grid_joint %>% 
  filter(N < 500) %>% 
  ggplot() +
  aes(N, theta, fill = density) +
  geom_raster() +
  ...
  labs(
    x = 'N',
    y = 'θ',
    title = 'Joint posterior distribution of N, θ',
    subtitle = 'p(N, θ | y) evaluated on a grid with non-negligible density'
  )
```
...
marginal %>% 
  ggplot() +
  aes(N, mass) +
  geom_col(fill = 'skyblue', colour = 'skyblue') +
  scale_y_continuous(labels = percent) +
  scale_x_continuous(
    labels = comma, 
    limits = c(max(counts$count), 1000),
    breaks = seq(0, 10000, 100)
  ) +
  labs(
    x = 'N',
    y = 'Mass',
    title = 'Marginal posterior distribution of N',
    subtitle = 'p(N | y)'
  )
```
